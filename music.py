    ...
    async def player_loop(self):
        ...
        while not self.bot.is_closed():
            ...
            try:
                ...
                async with async_timeout.timeout(40):  # 5 minutes...
            ...
            if not isinstance(source, YTDLSource):
                ...
                #     embed = discord.Embed(title="오류가 발생했어요!", description=f"오류내용: ```{str(e)}``` ", color=0xFF5900)
            ...
            try:
                ...
                embed = discord.Embed(title="알 수 없는 오류가 발생했어요", color=0xFF5900)
                await self._channel.send(embed=embed)
...
class Music(commands.Cog):
    ...
    async def __error(self, ctx, error):
        ...
        if isinstance(error, commands.NoPrivateMessage):
            try:
                return await ctx.send("❌DM채널에서는 사용할수 없는 명령어에요.")
            except discord.HTTPException:
                pass
        elif isinstance(error, InvalidVoiceChannel):
            embed = discord.Embed(title=":x: 음성채널에 접속하는데 문제가 발생했어요!", color=error_embed_color,
    ...
    async def connect_(self, ctx, *, channel: discord.VoiceChannel = None):
        if not channel:
            try:
                ...
                embed = discord.Embed(title=":x: 참여할 채널이 없어요!", color=error_embed_color,
        ...
        await ctx.send(f"**{channel}** 에서 노래를 들려드릴게요!")
    ...
    @commands.command(name="다시시작", aliases=["restart"])
    ...
    async def queue_info(self, ctx):
        vc = ctx.voice_client
        if not vc or not vc.is_connected():
            embed = discord.Embed(title=":x: 아직 음성채널에 접속하지 않았어요!", color=error_embed_color,
    ...
    async def stop_(self, ctx):
        ...
        embed = discord.Embed(title="재생중인 노래를 정지시키고 음성채널에서 나왔어요!", color=random_color())
        embed.set_footer(text=ctx.author.name, icon_url=ctx.author.avatar_url)
        embed.timestamp = datetime.datetime.utcnow()
        return await ctx.channel.send(embed=embed)

@bot.command(aliases=['진단등록', '진단토큰'])
async def hcs_token(ctx, name=None, birth=None, area=None, level=None, schoolname=None, password=None):
    if name == None or birth == None or area == None or level == None or schoolname == None or password == None:
        ...
        embed.add_field(name="이렇게 입력할 수 있어요",
                        value="```" + prefix + "진단등록 [이름] [생년월일 6자리] [지역] [학교급] [학교명] [자가진단 비밀번호]```", inline=True)
        embed.set_footer(text=ctx.author.name, icon_url=ctx.author.avatar_url)
        embed.timestamp = datetime.datetime.utcnow()
        return await ctx.channel.send(embed=embed)
    else:
...
@bot.command(aliases=['진단참여'])
async def hcs(ctx, hcs_token=None):
    if hcs_token == None:
        embed = discord.Embed(title=":x: 자가진단 토큰을 입력해 주세요", color=error_embed_color)
        embed.add_field(name="이렇게 입력할 수 있어요", value="```" + prefix + "진단참여 [토큰]```", inline=True)
        embed.set_footer(text=ctx.author.name, icon_url=ctx.author.avatar_url)
        embed.timestamp = datetime.datetime.utcnow()
        return await ctx.channel.send(embed=embed)
    else:
